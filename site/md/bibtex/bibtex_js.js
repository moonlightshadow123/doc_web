...
function BibtexDisplay() {
    ...
    this.createTemplate = function(entry, output) {
        ...
        tpl.attr("id","entry_"+entry["BIBTEXKEY"]);
        return tpl;
    }
    ...
    this.displayBibtex = function(input, output) {
        ...
        if (structure.length) {
            ...
            for (var entryKey in entries) {
                ...
                // Check if template was created
                if (tpl) {
                    output.children("ol").first().append("<li></li>").show();
                    output.children("ol").children("li").last().append(tpl);
                    tpl.show();
                    if (tpl.attr("callback")) {
                        var callback = new Function('bibtexentry', tpl.attr("callback"));
                        callback(tpl[0]);
...
function bibtex_js_draw(bib_str) {
    $(".bibtex_template").hide();
    //Gets the BibTex files and adds them together
    var bibstring = bib_str;
    var requests = [];
    ...
    var load = $.when.apply($, requests).then(function() {
...
function createWebPage(bib_str) {
...
function displayWithKeys(ekeys){
    var keys_str = "";
    ekeys.forEach(function(e, i){
        keys_str += ( e + "|");
    });
    keys_str = keys_str.substring(0, keys_str.length - 1);
    //console.log(keys_str);
    if(keys_str == ""){
        $("#bibtex_display").attr("bibtexkey", "_");
        $("#bibsect").hide();
    }else{
        $("#bibtex_display").attr("bibtexkey", keys_str);
    }
    /*
    var template_keys = "<div class=\"bibtex_template\""+ " bibtexkey=\""+ keys_str+ "\">" +
    "<div class=\"if author\" style=\"font-weight: bold;\">\n" +
    "<span class=\"if year\">\n" +
    "<span class=\"year\"></span>,\n" +
    "</span>\n  <span class=\"author\"></span>\n" +
